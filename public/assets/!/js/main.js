(function ($) {
    ...
    var country_id = $('.country_to_area').val();
    $.ajax({
        type : 'POST',
        url : page_data.routes.get_area_option_by_country,
        data : {country_id : country_id, _token : page_data.csrf_token},
        success: function(data){
            $('.area_options').html(data.area_options);
        }
    });

    $(document).on('change', '.country_to_area', function(e){
        e.preventDefault();
